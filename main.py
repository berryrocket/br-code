...
def InitBoard():
    ...
    timerAcq.init(freq=PARAMS.FREQ_ACQ, mode=Timer.PERIODIC, callback=Sampling)
...
def InitPlatFile():
    ...
    filePlatform.write(f"## Version soft : v{PARAMS.SOFT_VERSION:s}\n")
    filePlatform.write(f"## Type fusée : ")
    if (PARAMS.DEPOTAGE is True):
        filePlatform.write(f"Depotage\n")
    else:
        filePlatform.write(f"Trappe parachute\n")
    filePlatform.write(f"## Détection décollage : ")
    if (PARAMS.ACC_IMU is True):
        filePlatform.write(f"IMU mems (icm20948)\n")
    else:
        filePlatform.write(f"Accélero contact\n")
    filePlatform.write(f"## Fenetrage temporel : {PARAMS.TIMEOUT_FALLING:d} ms\n")
    filePlatform.write(f"## Frequence acq données: {PARAMS.FREQ_ACQ:d} Hz\n")
...
def FermetureParachute():
    if PARAMS.DEPOTAGE is False:
        global portePara
        portePara.duty_ns(PARAMS.SERVO_CLOSE*1000)

def OuvertureParachute():
    if PARAMS.DEPOTAGE is False:
        global portePara
        portePara.duty_ns(PARAMS.SERVO_OPEN*1000)
...
if __name__ == '__main__':
    ...
    while True:
        if isSampling is True:
            ...
            if ((ay < -1-PARAMS.ACC_THESHOLD and PARAMS.ACC_IMU is True) or (accContact is True and PARAMS.ACC_IMU is False)) and (isLaunched is False):
            ...
            if (isLaunched is True) and (isFalling is False):
                ...
                if (time.ticks_ms()-tempsDecollage > PARAMS.TIMEOUT_FALLING):
            ...
            if isLaunched is False:
                ...
                if (len(dataFilePlatBuff) > PARAMS.FREQ_ACQ/2):
